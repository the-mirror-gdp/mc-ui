...
class VectorInput
  extends Element
  implements IBindable, IFocusable, IPlaceholder
{
  ...
  protected _onValidate: (value: number[]) => boolean
  ...
  protected _onInputChange(input: NumericInput) {
    ...
    if (this._onValidate) {
      const error = !this._onValidate(this._inputs.map((input) => input.value))
      this.error = error
      if (error) {
        return
      }
    ...
    const multipleValues = this._inputs.some((input) =>
      input.class.contains(CLASS_MULTIPLE_VALUES)
    )
  ...
  link(observers: Observer | Observer[], paths: string | string[]) {
    ...
    const useSinglePath =
      paths.length === 1 || observers.length !== paths.length
    if (useSinglePath) {
      for (let i = 0; i < this._inputs.length; i++) {
    ...
    if (typeof value === 'string') {
      try {
        ...
        if (Array.isArray(value) && value.some((i) => !Number.isFinite(i))) {
          throw new Error(
            "VectorInput value set to string which doesn't contain an array of numbers"
          )
        }
      } catch (e) {
        console.error(e)
